...
move([TL, TR, BL, BR, IdElement, BX, BY, GX, GY, YX, YY, RX, RY],Path) :- 
	...
	nb_setval(tl,TL),  nb_setval(tr,TR), nb_setval(bl,BL), nb_setval(br,BR),
	 ...
	 element(IdElement, X, Y, TL, TR, BL, BR),
	 nb_setval(color,0),
	 writef('Robot bleu X=%d, Y=%d |PositionObstacle: %d / %d \n',[BX, BY, X, Y]),
...
move( [TL, TR, BL, BR, IdElement, BX, BY, GX, GY, YX, YY, RX, RY],Path) :-
	...
	element(IdElement, X, Y, TL, TR, BL, BR),
	writef('Robot vert X=%d, Y=%d |PositionObstacle: %d / %d \n',[GX, GY, X, Y]),
	nb_setval(color,1),
	selectRobot([BX,BY],[GX,GY],[YX,YY],[RX,RY],_),
...
nb_setval(tl,TL),  nb_setval(tr,TR), nb_setval(bl,BL), nb_setval(br,BR),
	...
	element(IdElement, X, Y, TL, TR, BL, BR),
	writef('Robot jaune X=%d, Y=%d |PositionObstacle: %d / %d \n',[YY, YY, X, Y]),
	nb_setval(color,2),
	selectRobot([BX,BY],[GX,GY],[YX,YY],[RX,RY],_),
...
move( [TL, TR, BL, BR, IdElement, BX, BY, GX, GY, YX, YY, RX, RY],Path) :-
	...
	element(IdElement, X, Y, TL, TR, BL, BR),
	writef('Robot rouge X=%d, Y=%d |PositionObstacle: %d / %d \n',[RX, RY, X, Y]),
	nb_setval(color,3),
	selectRobot([BX,BY],[GX,GY],[YX,YY],[RX,RY],_),
...
getAllAccessibleStates([[X,Y],_,_,_], AccessibleStatesList):-
		writef('getAllAccessibleStates à aprtir de %d:%d\n',[X,Y]),
		...
		writef('NX1:%d|NY1:%d \ NX2:%d|NY2:%d \ NX3:%d|NY3:%d \ NX4:%d|NY4:%d',[NX1,NY1,NX2,NY2,NX3,NY3,NX4,NY4]),
